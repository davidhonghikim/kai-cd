...
const PasswordGenerator: React.FC<PasswordGeneratorProps> = ({
  ...
  const analyzeCurrentPassword = () => {
    if (generatedPassword && onAnalyzePassword) {
      // Set the password in the analyzer store and call the callback
      setAnalyzedPassword(generatedPassword);
  ...
  const generateDiceware = async (preset?: keyof typeof DICEWARE_PRESETS, options?: DicewareOptions) => {
    try {
      ...
      // Store in security state
      setGeneratedPassword(result.passphrase, result);
  ...
  const generateLegacyPassword = () => {
    ...
    // Store in security state
    setGeneratedPassword(result, null);
  ...
  const updateCustomOptions = (newOptions: Partial<DicewareOptions>) => {
    const updatedOptions = { ...customDicewareOptions, ...newOptions };
    setCustomDicewareOptions(updatedOptions);
    updateActivity();
  };
